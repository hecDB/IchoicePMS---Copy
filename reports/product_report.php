...
$maxRangeDays = 90;
$today = date('Y-m-d');
$defaultFrom = date('Y-m-d', strtotime('-29 days')); // 30-day default window
...
try {
    $sql = "
        ...
        FROM issue_items ii
        INNER JOIN sales_orders so ON so.sale_order_id = ii.sale_order_id
        INNER JOIN products p ON p.product_id = ii.product_id
        WHERE DATE(so.sale_date) BETWEEN :date_from AND :date_to
        GROUP BY p.product_id, p.name, p.sku, p.barcode, p.image
    ...
    foreach ($rows as $row) {
        ...
        $imageSrc = resolveProductImage($row['image'] ?? '');

        $productData[] = [
            'name' => $row['name'] ?? 'ไม่ระบุชื่อสินค้า',
            'sku' => $row['sku'] ?? '-',
            'barcode' => $row['barcode'] ?? '-',
        ...
        $summaryTotals['orders'] += (int)$row['order_count'];
    ...
    error_log('Product report query failed: ' . $e->getMessage());
    $errors[] = 'ไม่สามารถดึงข้อมูลรายงานสินค้าได้ชั่วคราว กรุณาลองใหม่อีกครั้ง';
}
?>
<!DOCTYPE html>
<html lang="th">
<head>
    <meta charset="UTF-8">
    <title>รายงานสินค้า - IchoicePMS</title>
    ...
    <style>
        ...
        .report-table tbody td { padding: 12px 18px; border-bottom: 1px solid #e2e8f0; color: #1f2937; font-size: 14px; vertical-align: middle; }
        ...
        .product-cell img { width: 48px; height: 48px; border-radius: 12px; object-fit: cover; border: 1px solid #e2e8f0; background: #ffffff; }
        .product-cell .name { font-weight: 600; color: #1f2937; }
...
<body>
    <div class="mainwrap">
        <section class="report-header">
            ...
            <div>
                <h1>รายงานสินค้า</h1>
                <p>ตรวจสอบยอดขาย ต้นทุน และกำไรต่อสินค้าในช่วงเวลาที่เลือก (สูงสุด 90 วัน)</p>
            </div>
        </section>
        ...
        <form class="filters-card" method="get">
            ...
            <div class="field">
                ...
                <small>ระบบใช้อ้างอิงตามวันที่ขาย</small>
            </div>
            <button type="submit">
        ...
        <div class="summary-grid">
            <div class="summary-card">
                ...
                <strong><?= number_format(count($productData)) ?></strong>
                <span>จำกัดสูงสุด 50 รายการแรกตามยอดขาย</span>
            </div>
            <div class="summary-card">
                <h2>จำนวนคำสั่งซื้อ</h2>
                <strong><?= number_format($summaryTotals['orders']) ?></strong>
                <span>นับตามเลขแท็คที่มีสินค้านี้</span>
            </div>
            <div class="summary-card">
                <h2>จำนวนสินค้าที่ขาย</h2>
                <strong><?= number_format($summaryTotals['qty']) ?></strong>
                <span>รวมชิ้นสินค้าที่ถูกขาย</span>
            </div>
            <div class="summary-card">
                <h2>ยอดขายรวม</h2>
            ...
            <div class="summary-card">
                ...
                <span>หลังหักค่าธรรมเนียม 20% และต้นทุน</span>
            </div>
        </div>
        ...
        <?php if (!empty($productData)): ?>
            <article class="table-card">
                <header>
                    <h2><span class="material-icons" aria-hidden="true">inventory</span>สินค้า 50 รายการแรก</h2>
                ...
                <table class="report-table">
                    <thead>
                        <tr>
                            ...
                            <th>SKU</th>
                            <th>บาร์โค้ด</th>
                            <th class="numeric">จำนวนคำสั่งซื้อ</th>
                            ...
                            <tr>
                                <td>
                                    <div class="product-cell">
                                        <img src="<?= htmlspecialchars($row['image']) ?>" alt="ภาพสินค้า" onerror="this.src='../images/noimg.png'">
                                        <span class="name"><?= htmlspecialchars($row['name']) ?></span>
                                    </div>
                                </td>
                                <td><?= htmlspecialchars($row['sku']) ?></td>
                                <td><?= htmlspecialchars($row['barcode']) ?></td>
                                <td class="numeric"><?= number_format($row['orders']) ?></td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>—</td>
                            <td>—</td>
                            <td class="numeric"><?= number_format($summaryTotals['orders']) ?></td>
                ...
                <div class="report-notice">
                    รายงานนี้ใช้สมมติฐานค่าธรรมเนียม 20% เพื่อคำนวณกำไร หากอัตราจริงต่างออกไปสามารถปรับสูตรได้ในภายหลัง
                </div>
            </article>
        <?php else: ?>

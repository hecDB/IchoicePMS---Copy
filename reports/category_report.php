...
$monthsInRange = (($toObj->format('Y') - $fromObj->format('Y')) * 12) + ((int)$toObj->format('m') - (int)$fromObj->format('m')) + 1;
if ($monthsInRange < 1) {
    $monthsInRange = 1;
}
...
try {
    $sql = "
        SELECT 
            COALESCE(pc.category_name, p.category_name, 'ไม่ระบุหมวดหมู่') AS category_label,
        ...
        LEFT JOIN product_category pc ON pc.category_id = p.product_category_id
    ...
    foreach ($rows as $row) {
        ...
        $categoryData[] = [
            ...
            'cost' => $cost,
            'profit' => $profit,
            'avg_monthly_sales' => $monthsInRange > 0 ? ($sales / $monthsInRange) : 0.0
        ];
    ...
    error_log('Category report query failed: ' . $e->getMessage());
    $errors[] = 'ไม่สามารถดึงข้อมูลรายงานได้ชั่วคราว กรุณาลองใหม่อีกครั้ง';
}

$summaryAverageMonthlySales = $monthsInRange > 0 ? ($summaryTotals['sales'] / $monthsInRange) : 0.0;
?>
<!DOCTYPE html>
<html lang="th">
<head>
    <meta charset="UTF-8">
    <title>รายงานประเภทสินค้า - IchoicePMS</title>
    ...
    <style>
        ...
        .report-table .rank-col { width: 80px; text-align: center; font-weight: 600; color: #4f46e5; }
...
<body>
    <div class="mainwrap">
        <section class="report-header">
            ...
            <div>
                <h1>รายงานประเภทสินค้า</h1>
                <p>ติดตามประเภทสินค้าที่ขายดีที่สุดภายในช่วงเวลาที่เลือก (สูงสุด 90 วัน)</p>
            </div>
        </section>
        ...
        <div class="summary-grid">
            <div class="summary-card">
                <h2>ประเภทที่ดึงมา</h2>
                <strong><?= number_format(count($categoryData)) ?></strong>
                <span>เรียงลำดับจากยอดขายสูงสุด</span>
            </div>
            <div class="summary-card">
                <h2>จำนวนขายรวม</h2>
                <strong><?= number_format($summaryTotals['qty']) ?></strong>
                <span>หน่วยสินค้าที่ถูกขายทั้งหมด</span>
            </div>
            <div class="summary-card">
                <h2>ยอดขายรวม</h2>
        ...
        <?php if (!empty($categoryData)): ?>
            <article class="table-card">
                <header>
                    <h2><span class="material-icons" aria-hidden="true">leaderboard</span>20 ประเภทขายดี</h2>
                    <span>คำนวณกำไร = (ยอดขาย - 20%) - ต้นทุน</span>
                </header>
                <table class="report-table">
                    <thead>
                        <tr>
                            ...
                            <th>ประเภทสินค้า</th>
                            <th class="numeric">จำนวนขาย</th>
                            <th class="numeric">ยอดขาย (บาท)</th>
                            <th class="numeric">ต้นทุน (บาท)</th>
                            <th class="numeric">คำนวณกำไร (บาท)</th>
                            <th class="numeric">ยอดขายเฉลี่ยต่อเดือน (บาท)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($categoryData as $index => $row): ?>
                            <tr>
                                ...
                                <td class="numeric"><?= number_format($row['avg_monthly_sales'], 2) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td class="rank-col">รวม</td>
                            <td>ทุกประเภท</td>
                            ...
                            <td class="numeric"><?= number_format($summaryAverageMonthlySales, 2) ?></td>
                        </tr>
                    </tfoot>
                </table>
                <div class="report-notice">
                    ค่าธรรมเนียม 20% ถือเป็นสมมติฐานกลางสำหรับคำนวณกำไร หากอัตราจริงแตกต่าง สามารถปรับสูตรได้ภายหลัง
                </div>
            </article>
        <?php else: ?>
